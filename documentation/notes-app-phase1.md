# Notes Application - Phase 1: Frontend (React)
...
<html lang="en">
  <head>
    ...
    <meta
      name="description"
      content="Notes Application - Secure, Full-Stack"
    />
...
.note-form-section textarea:focus {
  outline: none;
  border-color: #667eea;
  box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1);
}
 ...
 * Simulates AWS Cognito behavior for local development
 * In production, this would use AWS Amplify or AWS SDK
 */

const MOCK_USERS = []; // In-memory user storage for mock
...
class AuthService {
   ...
   * @returns {Promise<object>} - Returns user info and token
   */
  ...
  async login(email, password) {
    ...
    // Find user (in production, query Cognito)
  ...
  _generateMockToken(userId, email) {
    ...
    // Simple base64 encoding (NOT cryptographically secure - for mock only)
...
/**
 * Note Card Component
 * Displays individual note with delete functionality
 */
  ...
  return (
    <div className="note-card">
      <div className="note-content">
        {note.title && <h3 style={{ marginBottom: '8px' }}>{note.title}</h3>}
        <p>{note.content}</p>
...
/**
 * Notes Page
 * Main application page - displays notes and form
 */
...
- **API Integration**: All requests include Authorization header with Bearer token
- **Responsive Design**: Mobile-friendly UI with gradient styling
- **Error Handling**: User-friendly error messages and loading states
- **State Management**: Uses React hooks for state and side effects

The application communicates exclusively with the backend at `http://localhost:8080/api` (proxied through webpack dev server).
